...
# 2. LINK DA SUA PLANILHA (Atualizado conforme sua imagem)
...
def carregar_dados():
    ...
    # Limpa a coluna de vidas para virar número (ex: "2 vidas" vira 2)
...
try:
    ...
    st.caption("Dados sincronizados em tempo real com o Google Sheets")
    ...
    with col_bar:
        ...
        fig_op = px.bar(df, x="QUAL O SEU PLANO DE SAÚDE ATUAL?", template="plotly_white", color_discrete_sequence=['#0ea5e9'])
    ...
    # 5. TABELA INTERATIVA (PLANILHA INTEIRA)
    st.divider()
    st.subheader("📋 Base de Dados Completa")
    
    busca = st.text_input("🔍 Digite para pesquisar (Nome, Cidade, Plano...)")
    ...
    st.dataframe(df_final.drop(columns=['vidas_num']), use_container_width=True)
    ...
    st.error("⚠️ Erro ao carregar os dados.")
    st.info("Verifique se a opção 'Restringir acesso' está DESMARCADA nas configurações de publicação da planilha.")
